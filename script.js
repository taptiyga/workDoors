    ...
    function createCard(config) {
      ...
      function updatePrice() {
        ...
        function hingePrice() { if (!hingeType || !hingeCount.value) return 0; let k = hingeType.value === "врезная" ? 0.4 : 0.2; return base * k * Number(hingeCount.value); }
        function lockPrice() { if (!lockType) return 0; if (lockType.value === "замок" || lockType.value === "защелка+задвижка") return base * 0.4; if (lockType.value === "защелка") return base * 0.2 * Number(hingeCount?.value || 1); return 0; }
